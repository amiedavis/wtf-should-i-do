...
class ActivityGeneratorView extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="activity-generator-view-container">
        ...
        <div className="pre-rating-container">{preRatingSection}</div>
      </div>
    );
  }
  ...
  setActivityNumber() {
    ...
    randomNumber =
      randomNumber === this.state.activityNumber
        ? randomNumber + 1
        : randomNumber;
...
function Activity(props) {
  return <div className="activity">{activities[props.value].activity}</div>;
}

function PositiveMessage() {
  const messages = ["You got this!", "F yeah!", "Awesome!", "Go you!"];
  const msgNo = Math.floor(Math.random() * 4);
  return <div className="positive-message">{messages[msgNo]}</div>;
}
